...
def _build_scout_enrich_prompt(scout: ScoutedLocation) -> str:
    ...
    return (
        ...
        "with varied queries. Also actively try to find a real, published email address for the "
        "business or the decision-maker — check the official website's contact/about page, staff "
        "directories, and business listings. Record an email ONLY if you actually see it published "
        "verbatim; NEVER guess, infer, or construct one from a name + domain pattern, and leave it "
        "blank rather than provide an address you cannot verify.\n\n"
        ...
        "  contact_email     - a real, published business or contact email, copied exactly as "
        "seen; empty string if you did not find a verifiable one (never fabricate or guess)\n"
